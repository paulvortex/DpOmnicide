...
// Fragment shader common definitions
...
vec4 fxaa(vec4 inColor, float maxspan)
{
	...
	vec3 rA = (1.0/2.0) * (texture2D(Texture_First, TexCoord1 + dir * (1.0/3.0 - 0.5)).xyz + texture2D(Texture_First, TexCoord1 + dir * (2.0/3.0 - 0.5)).xyz);
	vec3 rB = rA * (1.0/2.0) + (1.0/4.0) * (texture2D(Texture_First, TexCoord1 + dir * (0.0/3.0 - 0.5)).xyz + texture2D(Texture_First, TexCoord1 + dir * (3.0/3.0 - 0.5)).xyz);
...
#ifdef USEFXAA
	dp_FragColor = fxaa(dp_FragColor, 4.0); // 4.0 can be changed for larger span (span required for less pixel flickering during movement)
#endif
#ifdef USEBLOOM
...
vec3 FogVertex(vec4 surfacecolor, myhalf VertexAlpha)
{
	float FogPlaneVertexDist = EyeVector.w;
	vec3 fc = FogColor;
#ifdef USEFOGHEIGHTTEXTURE
	...
	return mix(fogheightpixel.rgb * fc, surfacecolor.rgb, dp_texture2D(Texture_FogMask, cast_myhalf2(length(EyeRelative)*fogfrac*FogRangeRecip, 0.0)).r);
#else
	#ifdef USEFOGOUTSIDE
	float fogfrac = min(0.0, FogPlaneVertexDist) / (FogPlaneVertexDist - FogPlaneViewDist) * min(1.0, min(0.0, FogPlaneVertexDist) * FogHeightFade);
	#else
	float fogfrac = FogPlaneViewDist / (FogPlaneViewDist - max(0.0, FogPlaneVertexDist)) * min(1.0, (min(0.0, FogPlaneVertexDist) + FogPlaneViewDist) * FogHeightFade);
	#endif
	return mix(fc, surfacecolor.rgb, dp_texture2D(Texture_FogMask, cast_myhalf2(length(EyeRelative)*fogfrac*(VertexAlpha + 0.25)*0.8*FogRangeRecip, 0.0)).r);
#endif
}
#endif
#ifdef USEGAMMARAMPS
...
#endif
	// calculate fresnel
	myhalf fresnel     = min(1.0, sunspecular + (min(1.0, 1.0 - float(eyenormal.z)) * ReflectFactor + ReflectOffset));
	// distort
...
#define WATER_REFRACTBLUR
	...
	dp_FragColor.rgb = mix(refract * mix(vec3(1.0), RefractColor.rgb, WaterFlow.a), dp_texture2D(Texture_Reflection, screentx.zw).rgb * ReflectColor.rgb, fresnel * waterflow_s) + SUNSPECULARCOLOR;
...
#ifdef USEFOG
	dp_FragColor.rgb = FogVertex(dp_FragColor, WaterFlow.a );
#endif
#ifdef USEGAMMARAMPS
	dp_FragColor.r = dp_texture2D(Texture_GammaRamps, vec2(dp_FragColor.r, 0)).r;
	dp_FragColor.g = dp_texture2D(Texture_GammaRamps, vec2(dp_FragColor.g, 0)).g;
...
vec3 FogVertex(vec4 surfacecolor)
{
...
#endif
#ifdef USEFOGHEIGHTTEXTURE
	vec4 fogheightpixel = dp_texture2D(Texture_FogHeightTexture, vec2(1,1) + vec2(EyeVectorFogDepth.w, FogPlaneViewDist) * (-2.0 * FogHeightFade));
	float fogfrac = fogheightpixel.a;
	...
	#endif
	#endif
	return mix(fc, surfacecolor.rgb, dp_texture2D(Texture_FogMask, cast_myhalf2(length(EyeVectorModelSpace)*FogRangeRecip, 0.0)).r);
#endif
}
#endif
...
#ifdef MODE_LIGHTMAP
	color.rgb = diffusetex * (Color_Ambient + DecodeLightmap(dp_texture2D(Texture_Lightmap, TexCoordSurfaceLightmap.zw)) * Color_Diffuse);
#endif // MODE_LIGHTMAP
...
#ifdef USEFOG
#ifdef SHOWFOG
	color.rgb = FogVertex(vec4(0.0, 0.0, 0.0, 1.0));
#else
